...
1. `git status`, `git log --oneline --decorate -10`, `git remote -v`. Expect a clean tree at
   `da4dcd2` or later, branch `main`, one remote `origin`. Five untracked local files are normal
   (`README.md` §5.4).
...
| Who did what, originally? | `LICENSE`, `git show 76fb7fd:README.md` (the original author's README: paper authors, contact, legacy app and CLI documented in full), `README.md` §1.2 |
